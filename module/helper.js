...
export class EntitySheetHelper {
  ...
  static onAttributeRoll(event) {
    ...
    const chatLabel = label ?? button.parentElement.querySelector(".attribute-key").value;
      const shorthand = game.settings.get("goldenSkyFanSystem", "macroShorthand");
  ...
  static async createDialog(data={}, options={}) {
    ...
    const collection = game.collections.get(this.documentName);
      const templates = collection.filter(a => a.getFlag("goldenSkyFanSystem", "isTemplate"));
      ...
      callback: html => {
        ...
        if ( template ) {
          ...
          createData.type = template.type;
            delete createData.flags.goldenSkyFanSystem.isTemplate;
        }
